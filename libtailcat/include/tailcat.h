...
// tailcat_client_public_key returns. Until any key is allowed, every
// client may connect; once one is, only allowed keys can. It may be called
...
// ("nodekey:<hex>") to buf, generating the ephemeral key first if none was
// set. Give this to the server's tailcat_server_allow_client.
